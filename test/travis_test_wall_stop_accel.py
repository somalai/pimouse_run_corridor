...
class WallStopAccelTest(unittest.TestCase):
	...
	def test_io(self):
		...
		self.assertTrue(left == 0 and right == 0, "can not stop")
		...
		self.assertTrue(left == right == 0, "can not stop")

if __name__ == '__main__':
	time.sleep(3)
	rospy.init_node('travis_test_wall_stop_accel')
	rostest.rosrun('pimouse_run_corridor', 'travis_test_wall_stop', WallStopAccelTest)
